...
class MessagePassing:
    ...
    async def send_post(self, url: str, data: Dict[str, Any], timeout: float = 2.0, headers: Dict[str, str] = None) -> Optional[Dict[str, Any]]:
        """Sends a POST request and returns the JSON response."""
        session = await self._get_session()
        try:
            async with session.post(url, json=data, timeout=timeout, headers=headers) as resp:
                # Return JSON even for non-200 status to allow status propagation (e.g., 404 Empty)
                return await resp.json()
        except Exception as e:
            logger.debug(f"Failed to send POST to {url}: {e}")
        return None

    async def send_get(self, url: str, timeout: float = 2.0, headers: Dict[str, str] = None) -> Optional[Dict[str, Any]]:
